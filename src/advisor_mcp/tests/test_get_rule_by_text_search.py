...
class TestGetRuleByTextSearch:
    ...
    async def test_get_rule_by_text_search_valid_text(self, advisor_mcp_server, advisor_mock_client, mock_api_response):
        ...
        with setup_advisor_mock(advisor_mcp_server, advisor_mock_client, mock_api_response):
            ...
            # Parse the result - use ast.literal_eval for Python string representation
            parsed_result = ast.literal_eval(result)
            assert parsed_result == mock_api_response

    @pytest.mark.asyncio
    async def test_get_rule_by_text_search_with_whitespace(
        self, advisor_mcp_server, advisor_mock_client, mock_api_response
    ):
        """Test get_rule_by_text_search with whitespace in search text."""
        search_text = "  filesystem performance  "
        ...
        with setup_advisor_mock(advisor_mcp_server, advisor_mock_client, mock_api_response):
            ...
            # Verify API was called with trimmed text
            advisor_mock_client.get.assert_called_once_with("rule/", params={"text": "filesystem performance"})
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("   ", "Error: Text search query must be a non-empty string."),
            (123, "Error: Text search query must be a non-empty string."),
        ],
    )
    @pytest.mark.asyncio
    ...
    async def test_get_rule_by_text_search_empty_response(self, advisor_mcp_server, advisor_mock_client):
        ...
        with setup_advisor_mock(advisor_mcp_server, advisor_mock_client, None):
            ...
            assert result == "No recommendations found for the given text search."

    @pytest.mark.asyncio
    async def test_get_rule_by_text_search_special_characters(
        self, advisor_mcp_server, advisor_mock_client, mock_api_response
    ):
        """Test get_rule_by_text_search with special characters."""
        search_text = "test-rule_name"
