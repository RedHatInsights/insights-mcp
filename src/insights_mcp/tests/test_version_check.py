...
from insights_mcp.server import get_insights_mcp_version


class TestVersionCheck:
    """Test class for version checking functionality."""
    ...
    def test_version_check_with_updates_available(
        self, mock_requests_get, mock_get_latest_release_tag, mock_github_api_response
    ):
        ...
        mock_requests_get.assert_called_once_with(
            "https://api.github.com/repos/RedHatInsights/insights-mcp/compare/20250905-001953-16930107...20250905-072605-a8f7bd3a"
        )


@pytest.mark.parametrize("version", ["20250905-072605-a8f7bd3a", "20240101-120000-abcdef12"])
class TestVersionCheckSameVersion:
    """Test class for version checking when current and latest versions are the same."""

    @patch("insights_mcp.server.get_latest_release_tag")
    def test_version_check_same_version(self, mock_get_latest_release_tag, version):
...
class TestVersionCheckRealAPI:
    """Integration tests for version checking with real GitHub API calls."""

    @patch("insights_mcp.server.__version__", "20250905-001953-16930107")
    def test_version_check_real_github_api(self):
    ...
    def test_version_check_real_api_same_version(self):
        """Test version checking with real API when versions might be the same."""
        # Get the actual latest release first
        from insights_mcp.server import get_latest_release_tag
    ...
    def test_get_latest_release_tag_real_api(self):
