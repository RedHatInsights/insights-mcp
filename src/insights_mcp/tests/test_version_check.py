...
@patch("insights_mcp.server.requests.get")
@pytest.mark.parametrize(
    "token_env,warning_name",
    [
        ("GITHUB_TOKEN", "GITHUB_TOKEN"),
        ("GH_TOKEN", "GH_TOKEN"),
    ],
)
def test_get_latest_release_tag_retries_without_token_on_401(
    mock_requests_get, monkeypatch, caplog, token_env: str, warning_name: str
):
    """Invalid env tokens must not block public GitHub endpoints."""
    ...
    ok_response.json.return_value = {"tag_name": "20250905-072605-a8f7bd3a"}
    mock_requests_get.side_effect = [unauthorized, ok_response]

    with caplog.at_level(logging.WARNING, logger="InsightsMCPServer"):
        latest_tag = get_latest_release_tag()

    assert latest_tag == "20250905-072605-a8f7bd3a"
    assert mock_requests_get.call_count == 2
    assert mock_requests_get.call_args_list[1].kwargs["headers"] == {
        "Accept": "application/vnd.github+json",
    }
    assert f"{warning_name} appears outdated or invalid" in caplog.text
